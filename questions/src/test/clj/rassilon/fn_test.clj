...
(future-fact "To call a function, we write a list of the form `(function arg1...argN)`"
  ((fn [a b] (+ a b)) 1 2)
  => ?????
...
(future-fact "Functions are first class, so we can assign them to let bindings or
...
(future-fact "Because defining top level functions is a rather common task, we can use
...
(future-fact "Multiple arities are supported, by using nested lists of ([args] & body)"
  (defn multi-add
    ([a]
       a)
    ([a b]
...
(future-fact "varargs are also supported, using `&`"
  (defn varargs-add
    [& args]
    (apply + args))
  => #'varargs-add
...
(future-fact "That last example used `apply`, which works much like `function.apply`
...
(future-fact "We can also use an anonymous function literal, which allows us to write
...
(future-fact "If we plan to use a single argument, we can use bare `%`"
  (#(+ % %) 3)
  => ?????)

(future-fact "rest args are still supported"
  (#(apply + %&) 1 2 3 4 5)
  => ?????)

(future-fact "Functions support pre and post conditions - % can be used inside `:post`
