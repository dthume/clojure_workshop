...
(fact "Note that vars are _not_ lexically scoped - def 'modifies' the current
...
are first class, and distinct from the values the point to enables a number
...
(fact "`let` introduces local, lexically scoped 'variables'."
  (let [a 1]
    a => 1)
...
(fact "because they're lexically scoped, they nest the way you'd expect"
  (let [a 1
        b 2
        c 3]
    (let [a 4]               ;; Shadows (does not change!) `a` from above
      (let [b 5]             ;; Shadows (does not change!) `b` from above
        (+ a b c) => 12)))
...
(fact "`let` bindings shadow global vars (from `def`)"
  (let [foo 1 bar 2]
    (+ foo bar) => 3))
